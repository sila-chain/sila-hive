...
For example, if you want to run the `discv4` test against sila, here is
how the command would look:

    ./hive --sim devp2p --sim.limit discv4 --client go-sila,nethermind
...
version by appending it to the client name with `_`, for example:
    ...
    ./hive --sim devp2p --client go-sila_v1.9.22,go-sila_v1.9.23
...
- `baseimage`: For clients pulled from DockerHub, this can be used to override the organization
   and image name. Example `sila/client-go`.
- `github`: For client Dockerfiles building from git, this setting can be used to change
   the source code repository (fork) on GitHub. Example: `sila-chain/go-sila`.
...
`sil` suite and selecting only tests containing the word `Large`.

    ./hive --sim devp2p --sim.limit sil/Large
...
directory (note the first `/`, matching any suite name):
    ...
    ./hive --sim sila/consensus --sim.limit /stBugs/
...
## Generating Sila 1.x test chains (hivechain)
