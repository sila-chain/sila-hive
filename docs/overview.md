...
of Sila clients and their features. Simulator programs usually don't need to care
...
any Sila 1 network definition, i.e. genesis block and hard fork activation block
...
Sila Foundation operates a public instance of Hive to check for consensus
compatibility, peer-to-peer networking spec compliance, and user API support for most
Sila client implementations.

You can find the latest test results at <https://hive.silapandaops.io>.
...
- `devp2p`: This simulator runs 'sil', 'snap' and 'discv4' peer-to-peer protocol tests.
  The test suites themselves are maintained in the go-sila repository. In their hive
...
- `sila/sync`: This simulator attempts to synchronize the blockchain among all
...
- `sila/consensus`: This simulator runs the Sila 1 consensus tests against all
...
- `sila/rpc`: The RPC simulator configures a client for clique PoA mining and runs
...
- `sila/graphql`: This simulator initializes a client with a known test chain and
  enables the GraphQL API endpoint. It then performs certain queries and compares their
  output to known good outputs.

- `sila/engine`: The engine API simulator verifies specification compliance of the
...
[Engine API]: https://github.com/sila-chain/execution-apis/tree/main/src/engine
...
```bash
./hive --sim sila/sync --client go-sila,besu,nethermind
```

Hive first builds simulator and client images using docker. It expects a Dockerfile in the
`./simulators/sila/sync` directory as well as a Dockerfile for each client (in
`./clients/*/Dockerfile`).
