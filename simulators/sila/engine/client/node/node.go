...
import (
	...
	beacon "github.com/sila-chain/go-sila/beacon/engine"
	...
	ethcatalyst "github.com/sila-chain/go-sila/sil/catalyst"
	"github.com/sila-chain/go-sila/sil/downloader"
	"github.com/sila-chain/go-sila/sil/ethconfig"
	ethprotocol "github.com/sila-chain/go-sila/sil/protocols/sil"
	...
	"github.com/sila-chain/sila-hive/simulators/sila/engine/client"
	typ "github.com/sila-chain/sila-hive/simulators/sila/engine/types"
...
func newNode(config GethNodeTestConfiguration, bootnodes []string, genesis *core.Genesis) (*GethNode, error) {
	// Define the basic configurations for the Sila node
...
func restart(startConfig GethNodeTestConfiguration, bootnodes []string, datadir string, genesis *core.Genesis) (*GethNode, error) {
	if startConfig.Name == "" {
		startConfig.Name = "Modified Sila Module"
	}
	config := &node.Config{
	...
	// Create the node and configure a full Sila node on it
	...
	ethBackend, err := sil.New(stack, econfig)
	if err != nil {
		return nil, err
	}
...
func (n *GethNode) ReOrgBackBlockChain(N uint64, currentBlock *types.Header) (*types.Header, error) {
	...
	for ; N > 0; N-- {
		currentBlock = n.sil.BlockChain().GetHeaderByHash(currentBlock.ParentHash)
		if currentBlock == nil {
			return nil, fmt.Errorf("Unable to re-org back")
		}
	}

	block := n.sil.BlockChain().GetBlock(currentBlock.Hash(), currentBlock.Number.Uint64())

	_, err := n.sil.BlockChain().SetCanonical(block)
...
func (n *GethNode) SetBlock(block *types.Block, parentNumber uint64, parentRoot common.Hash) error {
	...
	bc := n.sil.BlockChain()
	bc.SetBlockValidatorAndProcessorForTesting(new(validator), n.sil.BlockChain().Processor())
	...
	for _, candidateRoot := range candidateRoots {
		...
		result, err = n.sil.BlockChain().Processor().Process(block, statedb, *n.sil.BlockChain().GetVMConfig())
...
func (n *GethNode) BlockByNumber(ctx context.Context, number *big.Int) (*types.Block, error) {
	if number == nil && n.mustHeadBlock != nil {
		...
		block := n.sil.BlockChain().GetBlock(mustHeadHash, n.mustHeadBlock.Number.Uint64())
		return block, nil
	}
	return n.sil.APIBackend.BlockByNumber(ctx, parseBlockNumber(number))
}
...
func (n *GethNode) BlockNumber(ctx context.Context) (uint64, error) {
	...
	return n.sil.APIBackend.CurrentBlock().Number.Uint64(), nil
}

func (n *GethNode) BlockByHash(ctx context.Context, hash common.Hash) (*types.Block, error) {
	return n.sil.APIBackend.BlockByHash(ctx, hash)
}
...
func (n *GethNode) HeaderByNumber(ctx context.Context, number *big.Int) (*types.Header, error) {
	...
	b, err := n.sil.APIBackend.BlockByNumber(ctx, parseBlockNumber(number))
	if err != nil {
		return nil, err
	}
	if b == nil {
...
func (n *GethNode) HeaderByHash(ctx context.Context, hash common.Hash) (*types.Header, error) {
	b, err := n.sil.APIBackend.BlockByHash(ctx, hash)
	if err != nil {
		return nil, err
	}
	if b == nil {
...
func (n *GethNode) SendTransaction(ctx context.Context, tx typ.Transaction) error {
	if v, ok := tx.(*types.Transaction); ok {
		return n.sil.APIBackend.SendTx(ctx, v)
	}
...
func (n *GethNode) SendTransactions(ctx context.Context, txs ...typ.Transaction) []error {
	for _, tx := range txs {
		if v, ok := tx.(*types.Transaction); ok {
			err := n.sil.APIBackend.SendTx(ctx, v)
			if err != nil {
				return []error{err}
			}
		} else {
...
func (n *GethNode) getStateDB(ctx context.Context, blockNumber *big.Int) (*state.StateDB, error) {
	b, err := n.sil.APIBackend.BlockByNumber(ctx, parseBlockNumber(blockNumber))
	if err != nil {
		return nil, err
	}
	return state.New(b.Root(), n.sil.BlockChain().StateCache())
}
...
func (n *GethNode) GetLastAccountNonce(testCtx context.Context, account common.Address, head *types.Header) (uint64, error) {
	...
	if head == nil {
		block, err := n.sil.APIBackend.BlockByNumber(testCtx, rpc.LatestBlockNumber)
		if err != nil {
			return 0, err
		}
		head = block.Header()
...
func (n *GethNode) UpdateNonce(testCtx context.Context, account common.Address, newNonce uint64) error {
	// First get the current head of the client where we will send the tx
	head, err := n.sil.APIBackend.BlockByNumber(testCtx, rpc.LatestBlockNumber)
	if err != nil {
		return err
	}
	n.accTxInfoMap[account] = &AccountTransactionInfo{
