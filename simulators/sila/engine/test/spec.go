...
func (s BaseSpec) GetForkConfig() *config.ForkConfig {
	...
	if mainFork == config.SilaParis {
		if forkTime > genesisTimestamp || previousForkTime != 0 {
			return nil // Cannot configure a fork before SilaParis, skip test
		}
	} else if mainFork == config.SilaShanghai {
		if previousForkTime != 0 {
			return nil // Cannot configure a fork before SilaShanghai, skip test
		}
		forkConfig.ShanghaiTimestamp = new(big.Int).SetUint64(forkTime)
	} else if mainFork == config.SilaCancun {
