...
Given the following example,
    ...
    hive --client=clientA,clientB --sim=sila/engine
...
General positive and negative test cases based on the description in https://github.com/sila-chain/execution-apis/blob/main/src/engine/specification.md
...
### Sil RPC Status on ForkchoiceUpdated Events:
- `latest` Block after NewPayload:  
Verify the Block returned by the Sil RPC after a new payload is executed. Sil RPC should still return previous block.

- `latest` Block after New HeadBlockHash:  
Verify the Block returned by the Sil RPC after a new HeadBlockHash is set using forkchoiceUpdated. Sil RPC should return new block.

- `safe` Block after New SafeBlockHash:  
Verify the Block returned by the Sil RPC using the `safe` keyword after a new `SafeBlockHash` is set using `forkchoiceUpdated`. Sil RPC should return block set on the `SafeBlockHash`. When no block has been set as `SafeBlockHash` (`SafeBlockHash==0x00..00`), using `safe` shall return error.

- `finalized` Block after New FinalizedBlockHash:  
Verify the Block returned by the Sil RPC using the `finalized` keyword after a new `FinalizedBlockHash` is set using `forkchoiceUpdated`. Sil RPC should return block set on the `FinalizedBlockHash`. When no block has been set as `FinalizedBlockHash` (`FinalizedBlockHash==0x00..00`), using `finalized` shall return error.

- `safe`, `finalized` on Canonical Chain:
Verify the Block returned by the Sil RPC using the `finalized`/`safe` keyword after following the canonical chain is correctly updated to the canonical values set by `forkchoiceUpdated` calls.

- `latest` Block after Reorg:  
Verify the Block returned by the Sil RPC after a forkchoiceUpdated reorgs HeadBlockHash/SafeBlockHash to a sidechain and back. Sil RPC should return the appropriate block every time.
...
Attempt to produce a payload `P` after a transaction `INV_TX` with an invalid Chain ID was sent to the client using `sil_sendRawTransaction`.
...
b) Force the main client to obtain the chain via syncing by sending the entire invalid chain to a modified sila node, which is capable of accepting and relaying the invalid payloads.
...
Set the fee recipient to a custom address and verify that (a) balance is not increased when no fees are collected (b) balance is increased appropriately when fees are collected. Test using Legacy and SIP-1559 transaction types.
...
## Engine API SilaShanghai Upgrade Tests:
