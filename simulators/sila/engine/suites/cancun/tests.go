...
import (
	...
	"github.com/sila-chain/sila-hive/simulators/sila/engine/client/hive_rpc"
	...
	"github.com/sila-chain/sila-hive/simulators/sila/engine/globals"
	"github.com/sila-chain/sila-hive/simulators/sila/engine/helper"
	suite_engine "github.com/sila-chain/sila-hive/simulators/sila/engine/suites/engine"
...
// https://github.com/sila-chain/execution-apis/blob/main/src/engine/cancun.md
...
var Tests = []test.Spec{
	&CancunBaseSpec{
		...
		BaseSpec: test.BaseSpec{
			Name: "Blob Transactions On Block 1, SilaShanghai Genesis",
			About: `
			Tests the SilaCancun fork since Block 1.

			Verifications performed:
			- Correct implementation of Engine API changes for SilaCancun:
			  - engine_newPayloadV3, engine_forkchoiceUpdatedV3, engine_getPayloadV3
			- Correct implementation of SIP-4844:
			  - Blob transaction ordering and inclusion
			  - Blob transaction blob gas cost checks
			  - Verify Blob bundle on built payload
			- Sil RPC changes for SilaCancun:
			  - Blob fields in sil_getBlockByNumber
			  - Beacon root in sil_getBlockByNumber
			  - Blob fields in transaction receipts from sil_getTransactionReceipt
			`,
			MainFork: config.SilaCancun,
			// We fork after genesis
			ForkHeight: 1,
		},

		TestSequence: TestSequence{
			// We are starting at SilaShanghai genesis so send a couple payloads to reach the fork
			NewPayloads{},
	...
	&CancunBaseSpec{
		...
		BaseSpec: test.BaseSpec{
			Name: "Blob Transactions On Block 1, SilaCancun Genesis",
			About: `
			Tests the SilaCancun fork since genesis.

			Verifications performed:
			* See Blob Transactions On Block 1, SilaShanghai Genesis
			`,
			MainFork: config.SilaCancun,
		},
	...
	&CancunBaseSpec{
		BaseSpec: test.BaseSpec{
			Name: "ForkchoiceUpdatedV3 Set Head to SilaShanghai Payload, Null Payload Attributes",
			About: `
			Test sending ForkchoiceUpdatedV3 to set the head of the chain to a SilaShanghai payload:
			- Send NewPayloadV2 with SilaShanghai payload on block 1
		...
		TestSequence: TestSequence{
			NewPayloads{
				...
				ForkchoiceUpdatedV3 before SilaCancun returns no error without payload attributes
				`,
			},
		},
	},
	...
	&CancunBaseSpec{
		BaseSpec: test.BaseSpec{
			Name: "ForkchoiceUpdatedV3 To Request SilaShanghai Payload, Null Beacon Root",
			About: `
			Test sending ForkchoiceUpdatedV3 to request a SilaShanghai payload:
			- Payload Attributes uses SilaShanghai timestamp
		...
		TestSequence: TestSequence{
			NewPayloads{
				...
				ForkchoiceUpdatedV3 before SilaCancun with any null field must return INVALID_PAYLOAD_ATTRIBUTES (code %d)
	...
	&CancunBaseSpec{
		BaseSpec: test.BaseSpec{
			Name: "ForkchoiceUpdatedV3 To Request SilaShanghai Payload, Non-Null Beacon Root",
			About: `
			Test sending ForkchoiceUpdatedV3 to request a SilaShanghai payload:
			- Payload Attributes uses SilaShanghai timestamp
		...
		TestSequence: TestSequence{
			NewPayloads{
				...
				ExpectationDescription: fmt.Sprintf(`
				ForkchoiceUpdatedV3 before SilaCancun with beacon root field present must return UNSUPPORTED_FORK_ERROR (code %d)
	...
	&CancunBaseSpec{
		BaseSpec: test.BaseSpec{
			Name: "ForkchoiceUpdatedV2 To Request SilaShanghai Payload, Non-Null Beacon Root",
			About: `
			Test sending ForkchoiceUpdatedV2 to request a SilaShanghai payload:
			- Payload Attributes uses SilaShanghai timestamp
			- Payload Attributes Beacon Root is non-null
		...
		TestSequence: TestSequence{
			NewPayloads{
				...
				ExpectationDescription: fmt.Sprintf(`
				ForkchoiceUpdatedV2 before SilaCancun with beacon root field must return INVALID_PAYLOAD_ATTRIBUTES (code %d)
				`, *globals.INVALID_PAYLOAD_ATTRIBUTES),
			},
		},
	},
	...
	&CancunBaseSpec{
		BaseSpec: test.BaseSpec{
			Name: "ForkchoiceUpdatedV2 To Request SilaCancun Payload, Non-Null Beacon Root",
			About: `
			Test sending ForkchoiceUpdatedV2 to request a SilaCancun payload:
			- Payload Attributes uses SilaCancun timestamp
			- Payload Attributes Beacon Root is non-null
		...
		TestSequence: TestSequence{
			NewPayloads{
				...
				ExpectationDescription: fmt.Sprintf(`
				ForkchoiceUpdatedV2 after SilaCancun with beacon root field must return INVALID_PAYLOAD_ATTRIBUTES (code %d)
				`, *globals.INVALID_PAYLOAD_ATTRIBUTES),
			},
		},
	},
	&CancunBaseSpec{
		BaseSpec: test.BaseSpec{
			Name: "ForkchoiceUpdatedV2 To Request SilaCancun Payload, Missing Beacon Root",
			About: `
			Test sending ForkchoiceUpdatedV2 to request a SilaCancun payload:
			- Payload Attributes uses SilaCancun timestamp
		...
		TestSequence: TestSequence{
			NewPayloads{
				...
				ExpectationDescription: fmt.Sprintf(`
				ForkchoiceUpdatedV2 after SilaCancun must return UNSUPPORTED_FORK_ERROR (code %d)
				`, *globals.UNSUPPORTED_FORK_ERROR),
			},
		},
	},
	...
	&CancunBaseSpec{
		BaseSpec: test.BaseSpec{
			...
			About: `
			Test requesting a SilaCancun Payload using ForkchoiceUpdatedV3 twice with the beacon root
	...
	// GetPayloadV3 Before SilaCancun, Negative Tests
	&CancunBaseSpec{
		BaseSpec: test.BaseSpec{
			Name: "GetPayloadV3 To Request SilaShanghai Payload",
			About: `
			Test requesting a SilaShanghai PayloadID using GetPayloadV3.
			Verify that client returns UNSUPPORTED_FORK_ERROR.
			`,
			MainFork:   config.SilaCancun,
			ForkHeight: 2,
		...
		TestSequence: TestSequence{
			NewPayloads{
				...
				ExpectationDescription: fmt.Sprintf(`
				GetPayloadV3 To Request SilaShanghai Payload must return UNSUPPORTED_FORK_ERROR (code %d)
				`, *globals.UNSUPPORTED_FORK_ERROR),
			},
		},
	},

	// GetPayloadV2 After SilaCancun, Negative Tests
	&CancunBaseSpec{
		BaseSpec: test.BaseSpec{
			Name: "GetPayloadV2 To Request SilaCancun Payload",
			About: `
			Test requesting a SilaCancun PayloadID using GetPayloadV2.
			Verify that client returns UNSUPPORTED_FORK_ERROR.
			`,
			MainFork:   config.SilaCancun,
			ForkHeight: 1,
		...
		TestSequence: TestSequence{
			NewPayloads{
				...
				ExpectationDescription: fmt.Sprintf(`
				GetPayloadV2 To Request SilaCancun Payload must return UNSUPPORTED_FORK_ERROR (code %d)
				`, *globals.UNSUPPORTED_FORK_ERROR),
			},
		},
	},

	// NewPayloadV3 Before SilaCancun, Negative Tests
	&CancunBaseSpec{
		BaseSpec: test.BaseSpec{
			Name: "NewPayloadV3 Before SilaCancun, Nil Data Fields, Nil Versioned Hashes, Nil Beacon Root",
			About: `
			Test sending NewPayloadV3 Before SilaCancun with:
			- nil ExcessBlobGas
			- nil BlobGasUsed
		...
		TestSequence: TestSequence{
			NewPayloads{
				...
				ExpectationDescription: fmt.Sprintf(`
				NewPayloadV3 before SilaCancun with any nil field must return INVALID_PARAMS_ERROR (code %d)
	...
	&CancunBaseSpec{
		BaseSpec: test.BaseSpec{
			Name: "NewPayloadV3 Before SilaCancun, Nil ExcessBlobGas, 0x00 BlobGasUsed, Nil Versioned Hashes, Nil Beacon Root",
			About: `
			Test sending NewPayloadV3 Before SilaCancun with:
			- nil ExcessBlobGas
			- 0x00 BlobGasUsed
	...
	&CancunBaseSpec{
		BaseSpec: test.BaseSpec{
			Name: "NewPayloadV3 Before SilaCancun, 0x00 ExcessBlobGas, Nil BlobGasUsed, Nil Versioned Hashes, Nil Beacon Root",
			About: `
			Test sending NewPayloadV3 Before SilaCancun with:
			- 0x00 ExcessBlobGas
			- nil BlobGasUsed
	...
	&CancunBaseSpec{
		BaseSpec: test.BaseSpec{
			Name: "NewPayloadV3 Before SilaCancun, Nil Data Fields, Empty Array Versioned Hashes, Nil Beacon Root",
			About: `
				Test sending NewPayloadV3 Before SilaCancun with:
				- nil ExcessBlobGas
				- nil BlobGasUsed
	...
	&CancunBaseSpec{
		BaseSpec: test.BaseSpec{
			Name: "NewPayloadV3 Before SilaCancun, Nil Data Fields, Nil Versioned Hashes, Zero Beacon Root",
			About: `
			Test sending NewPayloadV3 Before SilaCancun with:
			- nil ExcessBlobGas
			- nil BlobGasUsed
	...
	&CancunBaseSpec{
		BaseSpec: test.BaseSpec{
			Name: "NewPayloadV3 Before SilaCancun, 0x00 Data Fields, Empty Array Versioned Hashes, Zero Beacon Root",
			About: `
			Test sending NewPayloadV3 Before SilaCancun with:
			- 0x00 ExcessBlobGas
			- 0x00 BlobGasUsed
		...
		TestSequence: TestSequence{
			NewPayloads{
				...
				ExpectationDescription: fmt.Sprintf(`
				NewPayloadV3 before SilaCancun with no nil fields must return UNSUPPORTED_FORK_ERROR (code %d)
				`, *globals.UNSUPPORTED_FORK_ERROR),
			},
		},
	},

	// NewPayloadV3 After SilaCancun, Negative Tests
	&CancunBaseSpec{
		BaseSpec: test.BaseSpec{
			Name: "NewPayloadV3 After SilaCancun, Nil ExcessBlobGas, 0x00 BlobGasUsed, Empty Array Versioned Hashes, Zero Beacon Root",
			About: `
			Test sending NewPayloadV3 After SilaCancun with:
			- nil ExcessBlobGas
			- 0x00 BlobGasUsed
			- Empty Versioned Hashes Array
			- Zero Beacon Root
		...
		TestSequence: TestSequence{
			NewPayloads{
				...
				ExpectationDescription: fmt.Sprintf(`
				NewPayloadV3 after SilaCancun with nil ExcessBlobGas must return INVALID_PARAMS_ERROR (code %d)
				`, *globals.INVALID_PARAMS_ERROR),
			},
		},
	},
	&CancunBaseSpec{
		BaseSpec: test.BaseSpec{
			Name: "NewPayloadV3 After SilaCancun, 0x00 ExcessBlobGas, Nil BlobGasUsed, Empty Array Versioned Hashes",
			About: `
			Test sending NewPayloadV3 After SilaCancun with:
			- 0x00 ExcessBlobGas
			- nil BlobGasUsed
		...
		TestSequence: TestSequence{
			NewPayloads{
				...
				ExpectationDescription: fmt.Sprintf(`
				NewPayloadV3 after SilaCancun with nil BlobGasUsed must return INVALID_PARAMS_ERROR (code %d)
				`, *globals.INVALID_PARAMS_ERROR),
			},
		},
	},
	&CancunBaseSpec{
		BaseSpec: test.BaseSpec{
			Name: "NewPayloadV3 After SilaCancun, 0x00 Blob Fields, Empty Array Versioned Hashes, Nil Beacon Root",
			About: `
			Test sending NewPayloadV3 After SilaCancun with:
			- 0x00 ExcessBlobGas
			- nil BlobGasUsed
		...
		TestSequence: TestSequence{
			NewPayloads{
				...
				ExpectationDescription: fmt.Sprintf(`
				NewPayloadV3 after SilaCancun with nil parentBeaconBlockRoot must return INVALID_PARAMS_ERROR (code %d)
				`, *globals.INVALID_PARAMS_ERROR),
			},
		},
	},
	...
	&CancunBaseSpec{
		BaseSpec: test.BaseSpec{
			...
			About: `
			Test requesting a SilaShanghai ForkchoiceUpdatedV2 payload followed by a SilaCancun ForkchoiceUpdatedV3 request.
			Verify that client correctly returns the SilaCancun payload.
			`,
			MainFork: config.SilaCancun,
		...
		TestSequence: TestSequence{
			NewPayloads{
				ExpectedIncludedBlobCount: 0,
				// This customizer only simulates requesting a SilaShanghai payload 1 second before cancun.
				// CL Mock will still request the SilaCancun payload afterwards
				FcUOnPayloadRequest: &helper.BaseForkchoiceUpdatedCustomizer{
	...
	&CancunBaseSpec{
		...
		TestSequence: TestSequence{
			...
			NewPayloads{
				...
				ExpectationDescription: `
				NewPayloadV3 after SilaCancun with nil VersionedHashes must return INVALID_PARAMS_ERROR (code -32602)
				`,
			},
		},
	},
	...
	// Most cases are contained in https://github.com/sila-chain/execution-spec-tests/tree/main/tests/cancun/sip4844_blobs
...
func init() {
	// Append all engine api tests with SilaCancun as main fork
	for _, test := range suite_engine.Tests {
		Tests = append(Tests, test.WithMainFork(config.SilaCancun))
	}

	// SilaCancun specific variants for pre-existing tests
	baseSpec := test.BaseSpec{
		MainFork: config.SilaCancun,
	}
