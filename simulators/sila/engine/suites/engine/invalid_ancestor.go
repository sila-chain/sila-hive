...
import (
	...
	"github.com/sila-chain/sila-hive/simulators/sila/engine/client/node"
...
func (tc InvalidMissingAncestorReOrgTest) Execute(t *test.Env) {
	...
	t.CLMock.ProduceBlocks(tc.SidechainLength, clmock.BlockProcessCallbacks{
		...
		OnPayloadProducerSelected: func() {
			...
			// Empty Txs Payload with invalid stateRoot discovered an issue in sila sync, hence this is customizable.
			if !tc.EmptyTransactions {
	...
	t.CLMock.ProduceSingleBlock(clmock.BlockProcessCallbacks{
		...
		OnGetPayload: func() {
			...
			if !tc.ReOrgFromCanonical {
				...
				defer cancel()
				l, err := t.Sil.BlockByNumber(ctx, nil)
				if err != nil {
			...
			for {
				...
				if r.Status.Status == test.Invalid {
					...
					ctx, cancel := context.WithTimeout(t.TestContext, globals.RPCTimeout)
					defer cancel()
					latestBlock, err := t.Sil.BlockByNumber(ctx, nil)
					if err != nil {
					...
					for ; k <= latestBlock.Number().Int64(); k++ {
						ctx, cancel = context.WithTimeout(t.TestContext, globals.RPCTimeout)
						defer cancel()
						latestBlock, err := t.Sil.BlockByNumber(ctx, big.NewInt(k))
						if err != nil {
