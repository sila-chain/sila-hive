...
type NewPooledTransactionHashes sil.NewPooledTransactionHashesPacket

func (msg NewPooledTransactionHashes) Code() int     { return 24 }
func (msg NewPooledTransactionHashes) ReqID() uint64 { return 0 }

type GetPooledTransactions sil.GetPooledTransactionsPacket
...
// PooledTransactionsPacket66 is the network packet for transaction distribution over sil/66.
...
func (c *Conn) Read() (Message, error) {
	...
	case (GetPooledTransactions{}.Code()):
		ethMsg := new(sil.GetPooledTransactionsPacket)
		if err := rlp.DecodeBytes(rawData, ethMsg); err != nil {
			return nil, errorf("could not rlp decode message: %v, %x", err, rawData)
		}
...
// Write writes a sil packet to the connection.
...
loop:
	...
	// make sure sil protocol version is set for negotiation
	if c.negotiatedProtoVersion == 0 {
		return nil, errors.New("sil protocol version must be set in Conn")
	}
...
func (c *Conn) readAndServe(timeout time.Duration) (Message, error) {
	...
	for time.Since(start) < timeout {
		...
		case *GetBlockHeaders:
			...
			resp := &BlockHeaders{
				RequestId:           msg.ReqID(),
				BlockHeadersRequest: sil.BlockHeadersRequest(headers),
			}
