...
func main() {
	suites := []hivesim.Suite{
		...
		makeSuite("legacy-cancun", "LegacyTests/SilaCancun/BlockchainTests"),
	}
	client := hivesim.New()
...
func makeSuite(name string, testsDirectory string) hivesim.Suite {
	suite := hivesim.Suite{
		...
		Description: "The '" + name + "' test suite executes BlockchainTests from the " +
			"official test repository (https://github.com/sila-chain/tests). For every test, it starts an instance of the client, " +
...
func testLink(filepath string) string {
	...
	// Link to
	// https://github.com/sila-chain/tests/blob/develop/BlockchainTests/InvalidBlocks/bcExpectSection/lastblockhash.json
	return fmt.Sprintf("https://github.com/sila-chain/tests/blob/develop/%v", path)
}
...
func getBlock(client *rpc.Client, arg string) (blockhash []byte, responseJSON string, err error) {
	...
	if err := client.Call(&blockData, "sil_getBlockByNumber", arg, false); err != nil {
