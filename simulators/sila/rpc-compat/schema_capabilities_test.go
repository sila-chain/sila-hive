...
// TestCapabilitiesSchemaValidation verifies that sil_capabilities responses
// with differing optional fields all validate against the OpenRPC result
// schema. sil_capabilities retention fields (oldestBlock, deleteStrategy) are
// client- and config-specific: e.g. an archive/hash sila omits deleteStrategy
// while a full/path sila includes it, and a client may disable a resource
...
func TestCapabilitiesSchemaValidation(t *testing.T) {
	...
	schema := schemas["sil_capabilities"]
	if schema == nil {
		t.Fatal("no schema for sil_capabilities")
	}

	head := `"head":{"number":"0x2d","hash":"0xe27a3e81bd7cfe2aec2cc9e832c73a17c93e7efcf659cf4b39883b96c48708c2"}`
	valid := map[string]string{
		// archive/hash sila: no deleteStrategy.
		...
		// full/path sila: adds deleteStrategy on state/tx/stateproofs.
