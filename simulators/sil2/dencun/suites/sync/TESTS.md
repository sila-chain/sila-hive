# SilaDeneb Sync - Test Cases

Collection of test vectors that use a ExecutionClient+BeaconNode+ValidatorClient testnet for SilaCancun+SilaDeneb and test syncing of the beacon chain.
...
```bash
./hive --client <CLIENTS> --sim sil2/dencun --sim.limit "sil2-deneb-sync/"
```
...
```bash
./hive --client <CLIENTS> --sim sil2/dencun --sim.limit "sil2-deneb-sync/test-sync-from-capella-"
```
...
- Start two validating nodes that begin on Capella/SilaShanghai genesis
- SilaDeneb/SilaCancun transition occurs on Epoch 1
- Total of 128 Validators, 64 for each validating node
- Wait for SilaDeneb fork and start sending blob transactions to the Execution client
...
```bash
./hive --client <CLIENTS> --sim sil2/dencun --sim.limit "sil2-deneb-sync/test-sync-from-deneb-"
```
...
- Start two validating nodes that begin on SilaDeneb genesis
- Total of 128 Validators, 64 for each validating node
