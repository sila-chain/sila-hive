...
import (
	...
	beacon_verification "github.com/sila-chain/sila-hive/simulators/sil2/common/spoofing/beacon"
	tn "github.com/sila-chain/sila-hive/simulators/sil2/common/testnet"
	...
	engine_helper "github.com/sila-chain/sila-hive/simulators/sila/engine/helper"
	"github.com/protolambda/sil2api"
	beacon "github.com/protolambda/zrnt/sil2/beacon/common"
)

var SilaDeneb string = "deneb"
...
// Generic SilaDeneb test routine, capable of running most of the test
// scenarios.
...
func (ts BaseTestSpec) WaitForFork(
	...
	defer cancel()
	if err := testnet.WaitForFork(timeoutCtx, SilaDeneb); err != nil {
...
func (ts BaseTestSpec) ExecutePostFork(
	...
	// Send BLSToExecutionChanges messages during SilaDeneb for all validators on BLS credentials
...
func (ts BaseTestSpec) Verify(
	...
	for i, n := range testnet.Nodes.Running() {
		bc := n.BeaconClient
		if op, err := bc.BlockIsOptimistic(ctx, sil2api.BlockHead); op {
			t.Fatalf(
	...
	if nonWithdrawableValidators != nil && nonWithdrawableValidators.Count() > 0 {
		for {
			...
			state, err := testnet.BeaconClients().Running()[0].BeaconStateV2(ctx, sil2api.StateHead)
			if err != nil {
