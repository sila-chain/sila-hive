...
import (
	...
	"github.com/protolambda/zrnt/sil2/beacon"
	"github.com/protolambda/zrnt/sil2/beacon/common"
	"github.com/protolambda/zrnt/sil2/configs"
	...
	cl_genesis "github.com/sila-chain/sila-hive/simulators/sil2/common/config/consensus/genesis"
	el "github.com/sila-chain/sila-hive/simulators/sil2/common/config/execution"
	"github.com/sila-chain/sila-hive/simulators/sil2/common/utils"
	beacon_client "github.com/marioevz/sil-clients/clients/beacon"
	exec_client "github.com/marioevz/sil-clients/clients/execution"
	validator_client "github.com/marioevz/sil-clients/clients/validator"
	mock_builder "github.com/marioevz/mock-builder/mock"
	builder_types "github.com/marioevz/mock-builder/types"
)

var (
	depositAddress                              common.Sil1Address
...
func PrepareTestnet(
	...
	sil1ConfigOpt := executionGenesis.ToParams(depositAddress)
	sil1Bundle, err := el.ExecutionBundle(executionGenesis.Genesis)
	if err != nil {
	...
	optimisticSync = optimisticSync.Set(
		"HIVE_SIL2_SAFE_SLOTS_TO_IMPORT_OPTIMISTICALLY",
	...
	for _, currentConfig := range []struct {
		...
		{
			ForkName:      "deneb",
			VersionConfig: &forkDecoder.SilaDeneb,
		},
	} {
		fmt.Printf(
			"Fork %s digest: %s\n",
	...
	commonOpts := hivesim.Params{
		"HIVE_SIL2_BN_GRPC_PORT": fmt.Sprintf(
			"%d",
			beacon_client.PortBeaconGRPC,
		),
		"HIVE_SIL2_METRICS_PORT": fmt.Sprintf(
			"%d",
			beacon_client.PortMetrics,
		),
		"HIVE_SIL2_CONFIG_DEPOSIT_CONTRACT_ADDRESS": depositAddress.String(),
		"HIVE_SIL2_DEPOSIT_DEPLOY_BLOCK_HASH": fmt.Sprintf(
			"%s",
			executionGenesis.Hash,
		),
	}
	beaconParams := hivesim.Params{
		...
		"HIVE_SIL2_MERGE_ENABLED": "1",
		"HIVE_SIL2_SIL1_GENESIS_TIME": fmt.Sprintf(
			"%d",
			executionGenesis.Genesis.Timestamp,
		),
		"HIVE_SIL2_GENESIS_FORK": config.GenesisBeaconFork(),
	}
	if config.DisablePeerScoring {
		beaconParams["HIVE_SIL2_DISABLE_PEER_SCORING"] = "1"
	}
...
func (p *PreparedTestnet) createTestnet(t *hivesim.T) *Testnet {
	...
	return &Testnet{
		...
		sil2GenesisState:      p.BeaconGenesis,
...
func (p *PreparedTestnet) prepareExecutionNode(
	...
	testnet *Testnet,
	sil1Def *hivesim.ClientDefinition,
	...
	cm.OptionsGenerator = func() ([]hivesim.StartOption, error) {
		...
		if len(currentlyRunningEcs) > 0 {
			...
			// Make the client connect to the first sil1 node, as a bootnode for the sil1 net
...
func (p *PreparedTestnet) prepareBeaconNode(
	...
	sil1Endpoints ...*exec_client.ExecutionClient,
	...
	cm.OptionsGenerator = func() ([]hivesim.StartOption, error) {
		...
		// Hook up beacon node to (maybe multiple) sil1 nodes
		var addrs []string
		var engineAddrs []string
		for _, en := range sil1Endpoints {
			...
			if err != nil {
				return nil, fmt.Errorf(
					"sil1 node used for beacon without available RPC: %v",
					err,
				)
			}
		...
		opts = append(opts, hivesim.Params{
			"HIVE_SIL2_SIL1_RPC_ADDRS":        strings.Join(addrs, ","),
			"HIVE_SIL2_SIL1_ENGINE_RPC_ADDRS": strings.Join(engineAddrs, ","),
			"HIVE_SIL2_BEACON_NODE_INDEX": fmt.Sprintf(
				"%d",
				config.ClientIndex,
			),
			"HIVE_SIL2_BN_API_PORT": fmt.Sprintf(
				"%d",
		...
		if len(currentlyRunningBcs) > 0 {
			if bootnodeENRs, err := currentlyRunningBcs.ENRs(parentCtx); err != nil {
				...
				opts = append(opts, hivesim.Params{"HIVE_SIL2_BOOTNODE_ENRS": bootnodeENRs})
			}
			...
			if staticPeers, err := currentlyRunningBcs.P2PAddrs(parentCtx); err != nil {
				...
				opts = append(opts, hivesim.Params{"HIVE_SIL2_STATIC_PEERS": staticPeers})
			}
		}
		...
		if cl.Builder != nil {
			if builder, ok := cl.Builder.(builder_types.Builder); ok {
				opts = append(opts, hivesim.Params{
					"HIVE_SIL2_BUILDER_ENDPOINT": builder.Address(),
				})
			} else {
		...
		// TODO
		//if p.configName != "sila-mainnet" && hasBuildTarget(beaconDef, p.configName) {
...
func (p *PreparedTestnet) prepareValidatorClient(
	...
	cm.OptionsGenerator = func() ([]hivesim.StartOption, error) {
		...
		bnAPIOpt := hivesim.Params{
			"HIVE_SIL2_BN_API_IP": bn.GetHost(),
		}
		bnAPIOpt = bnAPIOpt.Set(
			"HIVE_SIL2_BN_API_PORT",
		...
		// TODO
		//if p.configName != "sila-mainnet" && hasBuildTarget(validatorDef, p.configName) {
		//	opts = append(opts, hivesim.WithBuildTarget(p.configName))
		//}
		return opts, nil
	}
