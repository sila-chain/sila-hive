...
import (
	...
	ethcommon "github.com/sila-chain/go-sila/common"
	"github.com/sila-chain/go-sila/core"
	"github.com/pkg/errors"

	"github.com/protolambda/sil2api"
	"github.com/protolambda/zrnt/sil2/beacon/altair"
	"github.com/protolambda/zrnt/sil2/beacon/common"
	"github.com/protolambda/zrnt/sil2/beacon/phase0"
	"github.com/protolambda/zrnt/sil2/util/math"
	"github.com/protolambda/ztyp/tree"

	"github.com/sila-chain/sila-hive/hivesim"
	execution_config "github.com/sila-chain/sila-hive/simulators/sil2/common/config/execution"
	...
	node "github.com/marioevz/sil-clients/clients/node"
...
type Testnet struct {
	*hivesim.T
	...
	sil2GenesisState common.BeaconState
...
func StartTestnet(
	...
	if config.EnableBlobber {
		...
		prep.beaconOpts = hivesim.Bundle(prep.beaconOpts,
			hivesim.Params{
				"HIVE_SIL2_TRUSTED_PEER_IDS": strings.Join(ids, ","),
			})
	}
		...
		case tim := <-timer.C:
			...
			for i, n := range runningNodes {
				...
				go func(ctx context.Context, n *node.Node, r *result) {
					...
					headBlock, err := b.BlockV2(ctx, sil2api.BlockHead)
					if err != nil {
					...
					if (checkpoints.Finalized != common.Checkpoint{}) {
						if finalizedBlock, err := b.BlockV2(
							ctx,
							sil2api.BlockIdRoot(checkpoints.Finalized.Root),
						); err != nil {
							r.err = errors.Wrap(
								err,
		...
		case tim := <-timer.C:
			...
			for i, n := range runningNodes {
				...
				go func(ctx context.Context, n *node.Node, r *result) {
					...
					headInfo, err := b.BlockV2(ctx, sil2api.BlockHead)
					if err != nil {
						r.err = errors.Wrap(err, "failed to poll head")
						return
					}
		...
		case tim := <-timer.C:
			...
			if !ttdReached {
				...
				if td, err := executionClient.TotalDifficultyByNumber(ctx, nil); err == nil {
					...
					t.Logf("Error querying sil1 for TTD: %v", err)
				}
			}
...
func GetHealth(
	...
	var health float64
	stateInfo, err := bn.BeaconStateV2(parentCtx, sil2api.StateIdSlot(slot))
	if err != nil {
	...
	if currentEpochParticipation != nil {
		...
		validatorIds := make([]sil2api.ValidatorId, 0, len(state.Validators))
		...
		balancesBefore, err := bn.StateValidatorBalances(
			parentCtx,
			sil2api.StateIdSlot(beforeEpoch*int(spec.SLOTS_PER_EPOCH)),
			validatorIds,
		)
		if err != nil {
			return 0, fmt.Errorf(
		...
		balancesAfter, err := bn.StateValidatorBalances(
			parentCtx,
			sil2api.StateIdSlot(int(afterEpoch)*int(spec.SLOTS_PER_EPOCH)),
			validatorIds,
		)
		if err != nil {
			return 0, fmt.Errorf(
...
func legacyCalcHealth(
	spec *common.Spec,
	before, after []sil2api.ValidatorBalanceResponse,
) float64 {
