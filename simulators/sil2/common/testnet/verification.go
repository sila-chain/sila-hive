...
import (
	...
	"github.com/protolambda/zrnt/sil2/beacon/deneb"
...
func (l LastestSlotByHead) Slot(
	...
	headInfo, err := bn.BlockHeader(ctx, sil2api.BlockHead)
	if err != nil {
...
func (t *Testnet) VerifyExecutionPayloadIsCanonical(
	...
	for i, n := range runningNodes {
		...
		if block, err := ec.BlockByNumber(
			...
			return fmt.Errorf("sil1 %d: %s", 0, err)
		} else {
...
func (t *Testnet) VerifyExecutionPayloadHashInclusionNode(
	...
	for slot := lastSlot; slot > 0; slot -= 1 {
		versionedBlock, err := bn.BlockV2(parentCtx, sil2api.BlockIdSlot(slot))
		if err != nil {
			continue
		}
...
func (t *Testnet) VerifyProposers(
	...
	for slot := common.Slot(0); slot <= lastSlot; slot += 1 {
		...
		validator, err := bn.StateValidator(
			parentCtx,
			sil2api.StateIdSlot(slot),
			sil2api.ValidatorIdIndex(versionedBlock.ProposerIndex()),
		)
		if err != nil {
			return fmt.Errorf(
...
func (t *Testnet) VerifyELBlockLabels(parentCtx context.Context) error {
	...
	for i := 0; i < len(runningNodes); i++ {
		...
		bn := n.BeaconClient
		// Get the head
		headInfo, err := bn.BlockHeader(parentCtx, sil2api.BlockHead)
		if err != nil {
			return err
		}
...
func (t *Testnet) VerifyBlobs(
	...
	for slot := lastSlot; slot > 0; slot -= 1 {
		...
		versionedBlock, err := beaconClients[0].BlockV2(parentCtx, sil2api.BlockIdSlot(slot))
		if err != nil {
			continue
		}
		...
		refSidecars, err := beaconClients[0].BlobSidecars(parentCtx, sil2api.BlockIdSlot(slot))
		...
		for i := 1; i < len(beaconClients); i++ {
			...
			sidecars, err := bn.BlobSidecars(parentCtx, sil2api.BlockIdSlot(slot))
