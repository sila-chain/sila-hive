...
func PrintAllBeaconBlocks(
	...
	headInfo, err := b.BlockHeader(parentCtx, sil2api.BlockHead)
	...
	for i := 1; i <= int(headInfo.Header.Message.Slot); i++ {
		bHeader, err := b.BlockHeader(parentCtx, sil2api.BlockIdSlot(i))
		if err != nil {
		...
		if versionedBlock, err := b.BlockV2(parentCtx, sil2api.BlockIdRoot(root)); err == nil {
...
func PrintAllTestnetBeaconBlocks(
	...
	for nodeId, beaconNode := range runningBeacons {
		...
		var (
			nextBlock sil2api.BlockId
		)

		nextBlock = sil2api.BlockHead

		for {
			if nextBlock.BlockId() == sil2api.BlockIdRoot(tree.Root{}).
				BlockId() {
				break
			}
			// Get block header
			...
			if versionedBlock, err := beaconNode.BlockV2(parentCtx, sil2api.BlockIdRoot(root)); err == nil {
				if executionPayload, _, _, err := versionedBlock.ExecutionPayload(); err == nil {
			...
			nextBlock = sil2api.BlockIdRoot(parent)
		}
