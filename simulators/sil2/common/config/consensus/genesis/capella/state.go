...
import (
	...
	"github.com/protolambda/zrnt/sil2/beacon/capella"
...
func (g *GenesisStateView) EmptyBodyRoot() common.Root {
	return capella.BeaconBlockBodyType(configs.SilaMainnet).
		New().
...
func (g *GenesisStateView) SetGenesisExecutionHeader(
	...
	if executionGenesis.Difficulty().Cmp(ttd.ToBig()) < 0 {
		return fmt.Errorf(
			"sil1 genesis difficulty %d is less than terminal total difficulty %d",
	...
	execPayloadHeader := &capella.ExecutionPayloadHeader{
		...
		FeeRecipient:  common.Sil1Address(executionGenesis.Coinbase()),
