...
const (
	...
	EthGetBlockByHash         = "sil_getBlockByHash"
	EthGetBlockByNumber       = "sil_getBlockByNumber"
)
...
func (vs Signer) SignTx(
	...
	signer := types.NewSIP155Signer(vs.ChainID)
...
func (c BeaconCache) GetBlockStateByRoot(
	...
	b, err := bc.BlockV2(ctx, sil2api.BlockIdRoot(blockroot))
	if err != nil {
		return BeaconBlockState{}, err
	}
	s, err := bc.BeaconStateV2(ctx, sil2api.StateIdSlot(b.Slot()))
	if err != nil {
		return BeaconBlockState{}, err
	}
...
func (v *Validator) SignBLSToExecutionChange(
	...
	sil1Address := beacon.Sil1Address{}
	copy(sil1Address[:], executionAddress[:])
	blsToExecChange := beacon.BLSToExecutionChange{
		ValidatorIndex:     v.Index,
		FromBLSPubKey:      kdPubKey,
		ToExecutionAddress: sil1Address,
	}
...
func ValidatorFromBeaconValidator(
	...
	if wc[0] == beacon.SIL1_ADDRESS_WITHDRAWAL_PREFIX {
		withdrawAddress := common.Address{}
