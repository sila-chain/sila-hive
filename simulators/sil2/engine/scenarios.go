...
import (
	...
	api "github.com/sila-chain/go-sila/beacon/engine"
	"github.com/sila-chain/go-sila/common"
	"github.com/sila-chain/sila-hive/hivesim"
	"github.com/sila-chain/sila-hive/simulators/sil2/common/clients"
	"github.com/sila-chain/sila-hive/simulators/sil2/common/config"
	...
	"github.com/sila-chain/sila-hive/simulators/sil2/common/debug"
	payload_spoof "github.com/sila-chain/sila-hive/simulators/sil2/common/spoofing/payload"
...
var (
	...
	DEFAULT_CONFIG = &tn.Config{
		...
		Sil1Consensus: &el.ExecutionCliqueConsensus{},
	}
...
func Timeouts(t *hivesim.T, env *tn.Environment, n clients.NodeDefinition) {
	...
	config := getClientConfig(n).Join(&tn.Config{
		...
		// Use the default sila-mainnet slot time to allow the timeout value to make sense
		ConsensusConfig: &cl.ConsensusConfig{
...
func SyncingWithInvalidChain(
	...
	for _, bn := range testnet.VerificationNodes().BeaconClients().Running() {
		versionedBlock, err := bn.BlockV2(ctx, sil2api.BlockHead)
		if err != nil {
...
func NoViableHeadDueToOptimisticSync(
	...
	cfg := getClientConfig(n).Join(&tn.Config{
		...
		Sil1Consensus: el.ExecutionSilashConsensus{
			MiningNodes: 2,
		},
		ConsensusConfig: &cl.ConsensusConfig{
...
forloop:
	...
	for {
		...
		if opt, err := importer.BlockIsOptimistic(ctx, sil2api.BlockHead); err != nil {
			continue
		} else if opt {
