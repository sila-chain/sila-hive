...
builder_option=$([[ "$HIVE_SIL2_BUILDER_ENDPOINT" == "" ]] && echo "" || echo "--validators-proposer-blinded-blocks-enabled=true --validators-builder-registration-default-enabled=true")
echo $builder_option
...
/opt/teku/bin/teku vc \
    --network=auto \
    --beacon-node-api-endpoint="http://$HIVE_SIL2_BN_API_IP:$HIVE_SIL2_BN_API_PORT" \
    --data-path=/data/teku \
