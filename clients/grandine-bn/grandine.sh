...
if [ "$HIVE_SIL2_MERGE_ENABLED" != "" ]; then
    ...
    merge_option="--sil1-rpc-urls=$HIVE_SIL2_SIL1_ENGINE_RPC_ADDRS --jwt-secret=/jwtsecret"
fi

builder_option=$([[ "$HIVE_SIL2_BUILDER_ENDPOINT" == "" ]] && echo "" || echo "--builder-api-url=$HIVE_SIL2_BUILDER_ENDPOINT")
...
RUST_LOG=$LOG RUST_BACKTRACE=1 grandine \
	...
	--libp2p-port "${HIVE_SIL2_P2P_TCP_PORT:-9000}" \
	--discovery-port "${HIVE_SIL2_P2P_UDP_PORT:-9000}"  \
	--disable-enr-auto-update \
	--http-address 0.0.0.0 \
	--http-port ${HIVE_SIL2_BN_API_PORT:-4000} \
	...
	${HIVE_SIL2_BOOTNODE_ENRS:+--boot-nodes ${HIVE_SIL2_BOOTNODE_ENRS//,/ --boot-nodes }} \
	--configuration-file /data/testnet_setup/config.yaml \
	--deposit-contract-starting-block "${HIVE_SIL2_DEPOSIT_DEPLOY_BLOCK_NUMBER:-0}" \
