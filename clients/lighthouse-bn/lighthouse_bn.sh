...
echo "beacon index: $HIVE_SIL2_BEACON_NODE_INDEX"
if [[ "$HIVE_SIL2_BEACON_NODE_INDEX" != "" ]]; then
    pk="${pks[HIVE_SIL2_BEACON_NODE_INDEX]}"
    if [[ "$pk" != "" ]]; then
        key="$(echo $pk | xxd -r -p)"
...
if [[ "$HIVE_SIL2_TRUSTED_PEER_IDS" != "" ]]; then
    trustedpeers="$trustedpeers,$HIVE_SIL2_TRUSTED_PEER_IDS"
fi
...
metrics_option=$([[ "$HIVE_SIL2_METRICS_PORT" == "" ]] && echo "" || echo "--metrics --metrics-address=0.0.0.0 --metrics-port=$HIVE_SIL2_METRICS_PORT --metrics-allow-origin=*")
if [ "$HIVE_SIL2_MERGE_ENABLED" != "" ]; then
    echo -n "0x7365637265747365637265747365637265747365637265747365637265747365" > /jwtsecret
    merge_option="--execution-endpoints=$HIVE_SIL2_SIL1_ENGINE_RPC_ADDRS --jwt-secrets=/jwtsecret"
fi
opt_sync_option=$([[ "$HIVE_SIL2_SAFE_SLOTS_TO_IMPORT_OPTIMISTICALLY" == "" ]] && echo "" || echo "--safe-slots-to-import-optimistically=$HIVE_SIL2_SAFE_SLOTS_TO_IMPORT_OPTIMISTICALLY")
builder_option=$([[ "$HIVE_SIL2_BUILDER_ENDPOINT" == "" ]] && echo "" || echo "--builder=$HIVE_SIL2_BUILDER_ENDPOINT")
...
lighthouse \
    ...
    --enr-tcp-port="${HIVE_SIL2_P2P_TCP_PORT:-9000}" \
    --enr-udp-port="${HIVE_SIL2_P2P_UDP_PORT:-9000}" \
    --enr-address="${CONTAINER_IP}" \
    --disable-enr-auto-update=true  \
    --port="${HIVE_SIL2_P2P_TCP_PORT:-9000}" \
    --discovery-port="${HIVE_SIL2_P2P_UDP_PORT:-9000}"  \
    --target-peers="${HIVE_SIL2_P2P_TARGET_PEERS:-10}" \
    --trusted-peers="$trustedpeers" \
    --subscribe-all-subnets \
    --boot-nodes="${HIVE_SIL2_BOOTNODE_ENRS:-""}" \
    --max-skip-slots="${HIVE_SIL2_MAX_SKIP_SLOTS:-1000}" \
    --http --http-address=0.0.0.0 --http-port="${HIVE_SIL2_BN_API_PORT:-4000}" --http-allow-origin="*"
