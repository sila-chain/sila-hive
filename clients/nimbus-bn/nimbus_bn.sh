...
echo "${HIVE_SIL2_DEPOSIT_DEPLOY_BLOCK_NUMBER:-0}" > /data/testnet_setup/deposit_contract_block.txt
echo "${HIVE_SIL2_DEPOSIT_DEPLOY_BLOCK_HASH:-0}" > /data/testnet_setup/deposit_contract_block_hash.txt
...
metrics_option=$([[ "$HIVE_SIL2_METRICS_PORT" == "" ]] && echo "" || echo "--metrics --metrics-address=0.0.0.0 --metrics-port=$HIVE_SIL2_METRICS_PORT")

builder_option=$([[ "$HIVE_SIL2_BUILDER_ENDPOINT" == "" ]] && echo "" || echo "--payload-builder=true --payload-builder-url=$HIVE_SIL2_BUILDER_ENDPOINT")
...
/usr/bin/nimbus_beacon_node \
    ...
    --web3-url="$HIVE_SIL2_SIL1_ENGINE_RPC_ADDRS" \
    --jwt-secret=/jwtsecret \
    --num-threads=4 \
    $bootnodes_option $metrics_option $builder_option \
    --nat="extip:${CONTAINER_IP}" \
    --listen-address=0.0.0.0 \
    --tcp-port="${HIVE_SIL2_P2P_TCP_PORT:-9000}" \
    --udp-port="${HIVE_SIL2_P2P_UDP_PORT:-9000}" \
    --rest --rest-address=0.0.0.0 --rest-port="${HIVE_SIL2_BN_API_PORT:-4000}" --rest-allow-origin="*" \
    --doppelganger-detection=false \
    --max-peers="${HIVE_SIL2_P2P_TARGET_PEERS:-10}" \
