...
# Startup script to initialize and boot a go-sila instance.
#
# This script assumes the following files:
#  - `sila` binary is located in the filesystem root
...
if [ "$HIVE_NETWORK_ID" != "" ]; then
    ...
    # Unless otherwise specified by hive, we try to avoid sila-mainnet networkid. If sila detects sila-mainnet network id,
...
if [ -d /blocks ]; then
    (cd /blocks && $sila $FLAGS --gcmode=archive --verbosity=$HIVE_LOGLEVEL import --nocompaction `ls | sort -n`)
else
...
# Configure RPC.
FLAGS="$FLAGS --http --http.addr=0.0.0.0 --http.port=8545 --http.api=admin,debug,sil,miner,net,txpool,web3,testing"
FLAGS="$FLAGS --ws --ws.addr=0.0.0.0 --ws.origins \"*\" --ws.api=admin,debug,sil,miner,net,txpool,web3,testing"
