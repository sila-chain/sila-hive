...
func TestGenerateContainerName(t *testing.T) {
	name1 := GenerateContainerName("client", "go-sila")
...
func TestGenerateClientContainerName(t *testing.T) {
	name := GenerateClientContainerName("go-sila", TestSuiteID(1), TestID(2))
	...
	if !strings.Contains(name, "go-sila") {
...
func TestSanitizeContainerNameComponent(t *testing.T) {
	tests := []struct {
		...
		{"sila/rpc-compat", "sila-rpc-compat"},
		{"go-sila", "go-sila"},
		{"test_123", "test_123"},
...
func TestSanitizedContainerNames(t *testing.T) {
	...
	name := GenerateSimulatorContainerName("sila/rpc-compat")

	if strings.Contains(name, "/") {
		t.Errorf("Container name should not contain '/', got: %s", name)
	}

	if !strings.Contains(name, "sila-rpc-compat") {
