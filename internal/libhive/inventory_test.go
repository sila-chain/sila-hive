...
`
	...
	expectedOutput := []ClientDesignator{
		{Client: "go-sila", Nametag: "custom", DockerfileExt: "git", BuildArgs: map[string]string{"tag": "custom"}},
		{Client: "go-sila", DockerfileExt: "local"},
	...
	var inv Inventory
	inv.AddClient("go-sila", &InventoryClient{Dockerfiles: []string{"git", "local"}})
