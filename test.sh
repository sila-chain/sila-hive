...
function testconsensus {
  ...
  run "./hive --sim sila/consensus --client $client --sim.loglevel 6 --sim.testlimit 2 $FLAGS"
}
function testgraphql {
  echo "$(date) Starting graphql simulation [$1]"
  run "./hive --sim sila/graphql --client $1 $FLAGS"
}

function testsync {
  echo "$(date) Starting hive sync simulation [$1]"
  run "./hive --sim sila/sync --client=$1 $FLAGS"
}
...
#testsync go-sila_latest go-sila_stable
#testsync go-sila_latest nethermind_latest
#testsync go-sila_latest besu_latest

# GraphQL implemented only in besu and sila
#

testgraphql go-sila_latest
testgraphql besu_latest


# The devp2p tests are pretty quick -- a few minutes
#testdevp2p go-sila_latest
#testdevp2p nethermind_latest
#testdevp2p besu_latest


# These take an extremely long time to run
#testconsensus go-sila_latest
