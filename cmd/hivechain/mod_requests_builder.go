...
func init() {
	register("tx-request-sip8282-deposit", func() blockModifier {
		return &modBuilderRequest{deposit: true}
	})
	register("tx-request-sip8282-exit", func() blockModifier {
...
func (m *modBuilderRequest) apply(ctx *genBlockContext) bool {
	...
	if m.deposit {
		// SIP-8282 deposit input is pubkey || withdrawal_credentials ||
