...
`hive.silapandaops.io`; see below for pointing it at a local hive run.

## Install

```
go install github.com/sila-chain/sila-hive/cmd/hq@latest
```
...
`-client sila` resolves to `go-sila`, `-client nimbus` to `nimbus-el`.
...
hq runs -sim rpc-compat -client sila
```
...
hq tests -sim rpc-compat -client sila -failed

# Filter by test name (shell-style glob; * crosses /)
hq tests -sim rpc-compat -client sila -test "sil_getBalance*"
...
hq diff 1710000000-xxx.json -client sila
```
...
# Track sila's rpc-compat pass rate over time
hq stats -sim rpc-compat -client sila

# Look at more history
hq stats -sim rpc-compat -client sila -last 20
```
...
|------|---------|-------------|
| `-base-url` | `https://hive.silapandaops.io` | Hive server base URL |
